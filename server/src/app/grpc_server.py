...
class UserService(user_pb2_grpc.UserServiceServicer):
    ...
    def Login(self, request, context):
        print(f"Recibida solicitud de login: usuario={request.username}")
        if not self.login_use_case.login(request.username, request.password):
            print("Autenticación fallida")
            context.set_details("Login failed!")
            context.set_code(grpc.StatusCode.UNAUTHENTICATED)
            return user_pb2.LoginResponse(message="Login failed!")
    ...
    def CreateUser(self, request, context):
        ...
        if not success:
            ...
            return user_pb2.CreateUserResponse(message="User creation failed!")
...
def serve():
    ...
    user_pb2_grpc.add_UserServiceServicer_to_server(UserService(login_use_case,create_user), server)
